...
for (const width of [768, 1280]) {
  test(`split expands smoothly and keeps portrait visible at ${width}px`, async ({ page }) => {
    await page.setViewportSize({ width, height: 900 });
    await page.goto("/");
    ...
    await page.locator("summary").filter({ hasText: "Entreprise-grade" }).evaluate((el: HTMLElement) => el.click());
    ...
    await page.screenshot({ path: `.hig/evidence/motion-${width}.png`, fullPage: true });
  });
}

test("sheet follows dragging and settles without resizing", async ({ page }) => {
  ...
  await page.screenshot({ path: ".hig/evidence/motion-375.png", fullPage: true });

...
- Typography and links: self-hosted Satoshi variable font sitewide; body 400, UI labels medium (500); intro role muted tertiary, tagline at headline scale; readable clamp() sizes; hyperlinks grey by default, old rose (#C08081) on hover/active/focus, no underlines.
...
- Eight homepage dropdowns via `AnimatedDetails` + `DetailsAccordion`: 3 projects (Entreprise-grade Agents, Professional Tool for Animators, Offline Expo Navigation) + 5 jobs (OnDevice, Pluto, Kawa Space, Aula Education, Undergrad in Product) as two gap-separated lists (`home__project-groups`, no Projects/Experience section headings); work-ex titles are company name only; `+` when closed / `×` when open; inline body hyperlinks preferred over separate "View project" rows where user removed them; pullquotes for highlight lines.
...
- Homepage is `components/SimpleHome.tsx` via `app/page.tsx` — header contact from `content/site.ts` (`raashishah.work@gmail.com`, Calendly `https://calendly.com/raashishah-work/new-meeting`, Twitter `https://x.com/rash_driving` in footer); responsive grid (1 col <40rem, 2 col ≥40rem) with `projects` and `workExperience` arrays as eight `AnimatedDetails` dropdowns in two gap-separated lists, footer for remaining `socialLinks` (LinkedIn first); styles in `app/globals.css` (Apple tinted neutrals, 8pt spacing scale, 44px touch targets, safe-area insets, motion tokens); `viewport` export in `app/layout.tsx`.

...
Keep the Cal.com link off the default homepage. It appears after payment, on `/?mentoring=paid` when Dodo appends `status=succeeded`, and in the Dodo delivery email. The mentoring product’s digital delivery `external_url` and instructions are `https://cal.com/raashishah`.
